         ...
         <div className="lg:col-span-9 space-y-8">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-2">
               <div>
                  ...
                  <p className="text-[#8B95A5] font-medium leading-relaxed mt-2 text-sm max-w-md">
               ...
               <div className="flex bg-[#181D2A] border border-white/[0.04] p-1.5 rounded-2xl">
                  {['DISCOVER', 'CONNECTIONS', 'REQUESTS'].map((tab) => (
                    <button
                      key={tab}
                      onClick={() => setActiveTab(tab)}
                      className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-[#7B5CFA] text-white shadow-lg shadow-[#7B5CFA]/20' : 'text-[#5A6478] hover:text-white'}`}
                    >
                      {tab}
                     ...
                     <div className="space-y-8">
                        ...
                        <div className="flex flex-col lg:flex-row lg:items-center gap-4 justify-between">
                           <div className="flex overflow-x-auto no-scrollbar gap-2 pb-2 lg:pb-0">
                              {FILTERS.map(f => (
                                 <button 
                                   key={f}
                                   onClick={() => setFilter(f)}
                                   className={`flex-shrink-0 px-5 py-2.5 rounded-full text-xs font-black transition-all border ${filter === f ? 'bg-[#7B5CFA] text-white border-[#7B5CFA]' : 'bg-[#181D2A] text-[#8B95A5] border-white/[0.06] hover:border-white/20'}`}
                                 >
                                    {f}
                                 </button>
                              ))}
                           ...
                           <div className="relative group min-w-[250px]">
                              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-[#5A6478] group-focus-within:text-[#7B5CFA] transition-colors" size={16} />
                              <input 
                                type="text"
                                ...
                                className="w-full bg-[#181D2A] border border-white/[0.06] rounded-full py-2.5 pl-10 pr-5 text-xs font-bold focus:border-[#7B5CFA]/50 text-white outline-none transition-all shadow-inner"
                              />
                           </div>
                        </div>
